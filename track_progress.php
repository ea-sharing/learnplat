    ...
    <?php else: ?>
        <ul>
            <?php foreach ($completed_courses as $course): ?>
                <li>
                    <?= $course['course_name'] ?>
                    <!-- Add download link after the course name -->
                    <?php
                        // Fetch download link from certificates table for this course
                        $stmt = $pdo->prepare("SELECT download_link FROM certificates WHERE user_id = :userId AND course_id = :courseId");
                        $stmt->bindParam(":userId", $_SESSION['uid']);
                        $stmt->bindParam(":courseId", $course['course_id']);
                        $stmt->execute();
                        $certificate = $stmt->fetch(PDO::FETCH_ASSOC);
                        $downloadLink = $certificate ? $certificate['download_link'] : '';
                    ?>
                    <?php if ($downloadLink): ?>
                        <a href="<?= $downloadLink ?>"  target="_blank">Download Certificate</a>
                    <?php endif; ?>
                </li>
            <?php endforeach; ?>
        </ul>
